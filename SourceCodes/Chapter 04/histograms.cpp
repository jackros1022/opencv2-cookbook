...
int main()
{
	...
	cv::imshow("Binary Image",thresholded);
		//cv::imwrite("",thresholded);
    std::cerr << "Binary Image written to binary-output.jpg" << std::endl;
	...
	cv::imshow("Equalized Histogram",h.getHistogramImage(eq));
		...
		// Stretch the image ignoring bins with less than 500 pixels
	cv::Mat str= h.stretch(image,500);
    ...
    int dim=256;
    cv::Mat lookup(1, // dimension,
                   &dim, // 256 entries in the first dimension (row size)
                   CV_8U // uchar type
				   ); 	
	for (int i=0; i<256; i++)
