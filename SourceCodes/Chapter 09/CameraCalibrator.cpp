                                          ...
                                          cv::Size & boardSize)
{
    ...
    for (int i=0; i<boardSize.height; i++)
        for (int j=0; j<boardSize.width; j++)
            objectCorners.push_back(cv::Point3f(i/*X*/, j/*Y*/, 0.0f/*Z=0*/));
    ...
    for (int i=0; i<filelist.size(); i++) {
        // Open the image
        cv::Mat image = cv::imread(filelist[i],0/*Gray*/);
        if (image.empty()) {
        ...
        cv::cornerSubPix(image, imageCorners,
                         cv::Size(5,5), // half of the side length
                         cv::Size(-1,-1), // ?
        ...
        cv::imshow("Result from findChessboardCorners()", cimage);
        cv::waitKey(200);
    }
...
double CameraCalibrator::calibrate()
{
    ...
    for (int i=0; i<rvecs.size(); i++)
        cerr << i << "] rv:" << rvecs[i].t() << "  " << "t: " << tvecs[i].t() << endl;
    cerr << "-----------" << endl;
...
void  CameraCalibrator::testReprojection() {
    cerr << "! CameraCalibrator::testReprojection()" << endl;
    ...
    for (int i=0; i<srcImages.size(); i++) {
        ...
        for (int k=0; k<p.size(); k++) {
            ...
            double u = K(0,0)*xd + K(0,2); // now scale to image pixel coordinates
            double v = K(1,1)*yd + K(1,2); //
            
            double eu = u - p[k].x;
            double ev = v - p[k].y;
            double err = sqrt( eu*eu + ev*ev ); // reprojection error of the 3D point
            mean += err;
        
            float xud = K(0,2) + K(0,0)*Xc(0); // pixel coordinates, in the undistorted image
            float yud = K(1,2) + K(1,1)*Xc(1);
            
            cv::circle (undistortedImage, cv::Point2f(xud, yud), 3, cv::Scalar(0,0,255), 2);
        }
        cerr << i << "] --- " << mean/p.size() << endl;
