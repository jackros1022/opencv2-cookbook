...
public:
    ...
    void detect(const cv::Mat& image) {
        ...
        // Harris computation
        cv::cornerHarris(image,matCornerStrength,
        ...
        cv::minMaxLoc(matCornerStrength,&minStrength,&maxStrength);
        
        // local maxima detection
        cv::Mat matDilated;  // temporary image
        cv::dilate(matCornerStrength, matDilated,cv::Mat());
    ...
    cv::Mat getCornerMap(double qualityLevel) {
        ...
        cv::threshold(matCornerStrength,matCornerTh,threshold,255,cv::THRESH_BINARY);
        
        // convert to 8-bit image
        matCornerTh.convertTo(cornerMap,CV_8U);
        
        // non-maxima suppression was done already.
        // those contained in cornerMap will remain by the AND operation.
        cv::bitwise_and(cornerMap,matLocalMax,cornerMap);
