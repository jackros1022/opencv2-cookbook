...
int main()
{
	...
	cv::SurfFeatureDetector surf(500);
	...
	// Draw the kepoints
	cv::Mat imageKP;
	cv::drawKeypoints(image1,keypoints1,imageKP,cv::Scalar(5,255,255),cv::DrawMatchesFlags::DRAW_RICH_KEYPOINTS);
	cv::namedWindow("Right SURF Features");
	cv::imshow("Right SURF Features",imageKP);
	cv::drawKeypoints(image2,keypoints2,imageKP,cv::Scalar(5,255,255),cv::DrawMatchesFlags::DRAW_RICH_KEYPOINTS);
    ...
    for (int i=0; i<10; i++) {
        printf ("M[%d] = { %d %d %d %f }\n", i+1,
                matches[i].queryIdx, matches[i].trainIdx, matches[i].imgIdx, matches[i].distance);
    }
	cv::waitKey();
	return 0;
