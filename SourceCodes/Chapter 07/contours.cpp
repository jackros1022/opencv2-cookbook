...
int main()
{
	...
	cv::Mat canny_contours, canny_contours7;
	cv::Canny(image,canny_contours,125,350);
	cv::Mat contoursInv;
	cv::threshold(canny_contours,contoursInv,128,255,cv::THRESH_BINARY_INV);
    // Display the image of contours
	cv::namedWindow("Canny Contours");
	cv::imshow("Canny Contours",contoursInv);

    /****
	cv::Canny(image,canny_contours7, 10000,15000, 7);
	cv::Mat contoursInv7;
	cv::threshold(canny_contours7, contoursInv7,128,255,cv::THRESH_BINARY_INV);
  	cv::namedWindow("Canny Contours 7");
	cv::imshow("Canny Contours 7",contoursInv7);
  	cv::namedWindow("Canny output 7");
	cv::imshow("Canny output 7", canny_contours7);
    ****/
	...
	cv::HoughLines(canny_contours,lines,1,PI/180,60);
    
	// Draw the lines
	cv::Mat result(canny_contours.rows,canny_contours.cols,CV_8UC3,cv::Scalar(255,255,255));
	//image.copyTo(result);
	...
	while (it!=lines.end()) {
		...
		if (theta < PI/4. || theta > 3.*PI/4.) { // ~vertical line
			...
			// draw a white line
			cv::line( result, pt1, pt2, cv::Scalar(0,255,255), 1); 
		}
	...
	// Detect lines
	std::vector<cv::Vec4i> li= ld.findLines(canny_contours);
	...
	cv::bitwise_and(canny_contours,oneline,oneline);
	cv::Mat onelineInv;
